...
def validate_order(order):
    ...
    if order.get('order_id') is None:
        errors.append(f"Zamówienie ID: {order.get('order_id', 'BRAK')} - Brakuje klucza zamówienia")
    ...
    if raw_order_value is None:
        ...
        if order_value is None:
            ...
            errors.append(f"Zamówienie o ID: {order.get('order_id', 'BRAK')} - Wartość zamówienia mniejsza lub równa 0")
        
    raw_status = order.get("status")
    status = normalize_status(raw_status)

    if status is None:
        errors.append(f"Zamówienie o ID: {order.get('order_id', 'BRAK')} - Brakuja klucza statusu zamówienia")
    elif not is_valid_status(status):
        errors.append(f"Zamówienie o ID: {order.get("order_id", "BRAK")} - Niedozowolony status")
